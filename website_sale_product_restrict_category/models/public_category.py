        ...
        if vals.get("partner_ids"):
            ...
            for partner in partner_ids:
                ...
                for child_category in child_category_ids:
                    partner_ids = (
                        child_category.partner_ids
                        and set(child_category.partner_ids + partner)
                        or partner
                    )
                    child_category.write({"partner_ids": [(6, 0, partner_ids.ids)]})
                public_category = (
                    partner.mapped("public_category_ids")
                    and partner.mapped("public_category_ids").ids
                    or []
                )
        ...
        if vals.get("partner_ids"):
            ...
            for partner in partner_ids:
                ...
                for child_category in child_category_ids:
                    partner_ids = (
                        child_category.partner_ids
                        and child_category.partner_ids + partner
                        or partner
                    )
                    child_category.write({"partner_ids": [(6, 0, partner_ids.ids)]})
                public_category = (

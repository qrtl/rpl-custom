...
class ProductAttributeValuePermission(models.Model):
    ...
    def _check_partner_ids(self):
        if self.allowed_partner_ids and self.unallowed_partner_ids:
            ...
            union_list = list(set(self.allowed_partner_ids + self.unallowed_partner_ids))
            if len(concat_list) != len(union_list):
                raise ValidationError(_("Partner can only be eirther allowed or unallowed under the same rule."))
    ...
    def _check_country_ids(self):
        if self.allowed_country_ids and self.unallowed_country_ids:
            ...
            union_list = list(set(self.allowed_country_ids + self.unallowed_country_ids))
            if len(concat_list) != len(union_list):
                raise ValidationError(_("Country can only be either allowed or unallowed under the same rule."))
    ...
    def _check_country_group_ids(self):
        if self.allowed_country_group_ids and self.unallowed_country_group_ids:
            concat_list = self.allowed_country_group_ids + self.unallowed_country_group_ids
            union_list = list(set(self.allowed_country_group_ids + self.unallowed_country_group_ids))
            if len(concat_list) != len(union_list):
                raise ValidationError(_("Country group can only be either allowed or unallowed under the same rule."))

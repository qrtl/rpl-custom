...
class StockProductionLot(models.Model):
    ...
    def _calculate_expiry_date(self, product_id=None, date=None):
        date = date or datetime.now()
        p = self.env["product.product"].browse(product_id) or self.product_id
        if p and p.base_date or p.month_delta or p.weekday_delta or p.day_delta:
            ...
            if p.weekday_delta:
                ...
                weekday_delta = (
                    adjusted_delta if date.weekday() == 0 else p.weekday_delta
                )

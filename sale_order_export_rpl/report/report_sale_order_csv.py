    ...
    def generate_csv_report(self, writer, data, orders):
        ...
        for order in orders:
            writer.writerow({
                ...
                'Date': order.confirmation_date and order.confirmation_date.strftime("%d/%m/%Y %H:%M") or "",
                'Status': order.luxis_status or self.env['ir.config_parameter'].sudo().get_param('sale_order_export_rpl.luxis_status', default=""),
                'Notes': order.note,
                'Payment ID': self.env['ir.config_parameter'].sudo().get_param('sale_order_export_rpl.luxis_payment_id', default=12),
                'IP address': self.env['ir.config_parameter'].sudo().get_param('sale_order_export_rpl.luxis_ip_address', default=""),
                'Details': order.note2,
                'Payment information': "",
                'Taxes': "",
                'Coupons': "",
                ...
                'First name': "", 
                'Last name': "",
                'Company': order.partner_id.parent_id and order.user_id.partner_id.parent_id.name,
                'Fax': "",
                'Phone': order.partner_shipping_id.phone,
                'Web site': "",
                'Tax exempt': "N",
                'Language': self.env['ir.config_parameter'].sudo().get_param('sale_order_export_rpl.luxis_language', default=""),
                # 請求先情報
                'Billing: title': order.partner_invoice_id.title and order.partner_invoice_id.title.name or "",
                ...
                'Billing: state': order.partner_invoice_id.state_id and order.partner_invoice_id.state_id.code or "",
                'Billing: country': order.partner_invoice_id.country_id and order.partner_invoice_id.country_id.code or "",
                'Billing: zipcode': order.partner_invoice_id.zip,
                'Shipping: title': order.partner_shipping_id.title and order.partner_shipping_id.title.name or "",
                # 配送先情報
                ...
                'Shipping: address (line 2)': order.partner_shipping_id.street2 or "",
                'Shipping: city': order.partner_shipping_id.city or "",
                'Shipping: state': order.partner_shipping_id.state_id and order.partner_shipping_id.state_id.code or "",
                'Shipping: country': order.partner_shipping_id.country_id and order.partner_shipping_id.country_id.code or "",
